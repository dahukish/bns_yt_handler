//$Rev: 34283 - async=true
//<![CDATA[
if(!window.mmcore){
window.mmcore={
	domain:'scotiabank.com',
	server:'service.maxymiser.net/cg/v5us/',
	cookie_domain:'.scotiabank.com',
	tpixel:false,
	cprefix:'mmcore.',
	inline_state:true,
	OPC_install:function(){var t=this;t.SetParam('OPC.install','1');t.tpixel=false;t.CGRequest()},
	...
	AddDocLoadHandler:function(handler){var t=this,d=t._d,f,tm1,u=t._L(t._nua);
...
//transport
	ProlongRequest:function(){var a=mmcore;a.cS=a._request_id;clearTimeout(a.tm);a.tm=setTimeout(a.ShowXHTML, 5E3);},
	XHMLloaded:function(id){var a=mmcore;a.onloadDone=true;if(a.cS != id)a.ShowXHTML();},
	AttachRules:function(c){try{var b=document.createElement("style");b.type="text/css";/WebKit|MSIE/i.test(mmcore._nua)?b.styleSheet?b.styleSheet.cssText=c:b.innerText=c:b.innerHTML=c;mmcore._AddScriptNode(b)}catch(a){}},
	HideXHTML:function(){mmcore.AttachRules("html{left:-10000px;position:absolute;top:-10000px;}")},
	ShowXHTML:function(){if(!mmcore._shown){mmcore.AttachRules("html{position:static;}");mmcore._shown=1}},
	timeout:function(){this.ShowXHTML()},
	onloadDone:false,
	_rt:1000,
//private
	jsver:'5.12',_vars:{fv:{},act:{},uat:{},ids:{}},_act_id:0,_vars_alias:{act:'uv'},_renderers:{},_extensions:{},_r_mbs:{},
	_w:window,_d:document,_undef:undefined,_callback:[],_request_id:0,
	_filters:[],
	_sid:[],_rd:null,_docEnd:false,_docEndF:[],_incrRender:true,_nua:navigator.userAgent,
	...
	_AddScriptNode:function(p){
		var i,n,s=document.getElementsByTagName('script');for(i=s.length;i--;){if(s[i].src&&/mmcore\.js$/.test(s[i].src))n=s[i]}if(n){n.parentNode.insertBefore(p,n)}
	},
	_Tag:function(uri){
		var _t=this,_n,px=_t.tpixel,tp='text/javascript',d=_t._d,_a,_r_id=_t._request_id,id=_t.cprefix+_r_id;if(_t.GetParam('gm')=='2')return;
		_n=d.createElement(px?'img':'script');_n.id=id;_n.src=uri;if(!px){_n.type=tp;_n.charset='utf-8'};
		if(px){_a=d.getElementsByTagName('body');if(_a&&_a.length)_a[0].appendChild(_n);}else{_t._AddScriptNode(_n)}
		if(!_t.onloadDone){
			var loadCB = function(){_t.XHMLloaded(_r_id)};
			_n.onload=loadCB;
			_n.onerror=loadCB;
			_n.onreadystatechange=function(){if("loaded"===_n.readyState||"complete"===_n.readyState){setTimeout(loadCB, 0);}}
		}_t._Clear();return id},
	...
	_MbStyle:function(c,b){for(var a="",d=0;d<b.length;d++)a+=(d>0?",#":"#")+b[d];a+=c;mmcore.AttachRules(a)},
	_InitVcb:function(u,cid){var _t=this,_d=_t._d,_vs; _vs=_d.createElement("script");_vs.type='text/javascript';_vs.src=unescape(u+'?cid='+cid);_vs.charset='utf-8';_t._AddScriptNode(_vs);_t.SetParam('gm',2);_t.SetCookie('cfgid',1,0);},
	...
	_Init:function(first){
		...
		if(_t.GetParam('gm')!='2'){_t.tm = setTimeout(mmcore.ShowXHTML, 5E3);mmcore.HideXHTML()};
...
if (/Opera/i.test(mmcore._nua)) mmcore.CGRequest=function (args) {return false};
else window.mmcore._Init(true);
if(window.mmPageID) mmcore.SetPageID(window.mmPageID);
if (/Firefox/.test(mmcore._nua)) mmcore.AddDocLoadHandler(function(){});
