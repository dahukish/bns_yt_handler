...
var s_codeVersion="SB v2.0|20130529|H26"
/* setting s_account at the page level, if not set, set to catchall report suite*/
if(!s_account){var s_account="scotiabankcatchall";}
...
var acctmatch=s_account.toLowerCase();
if(urlSite.indexOf('apps.scotiabank.com')!=-1){s.charSet='ISO-8859-1';}
else if(acctmatch.indexOf('scotiabankonline')!=-1){s.charSet='ISO-8859-1';}
...
s.linkInternalFilters="javascript:,scotiabank.com,scotiabank.mobi,scotiaitrade.com,scotialifefinancial.com,roynat.com"
...
s.visitorNamespace="scotiabank"
s.dc=122

/*PLEASE UNCOMMENT THIS AFTER THE SSL CERTIFICATE HAS BEEN INSTALLED*/
s.trackingServer="omniture.scotiabank.com"
s.trackingServerSecure="somniture.scotiabank.com"


/* Form Analysis Config */
s.formList="" /*comma delimited, populate with form names to track fallout on*/
...
function s_doPlugins(s) {
	/* populate campaign variable based on query string paramter "cid", only populate once */
	if(!s.campaign){s.campaign=s.getQueryParam('cid')}
	s.campaign=s.getValOnce(s.campaign,"s_campaign",0)
    /*prop33 is to be used in Pathing reports for Campaign Path Analysis */
	if(s.campaign){s.prop33=s.campaign+':'+s.pageName;}
	else {s.prop33=s.pageName;}
	/* populate eVar2 variable based on query string paramter "intcmp", only populate once */
	if(!s.eVar2){s.eVar2=s.getQueryParam('intcmp')}
	s.eVar2=s.getValOnce(s.eVar2,"s_intcmp",0)
	/* if internal campaign (eVar2), set internal clickthrough event (event11) */
	if(s.eVar2) {
		var t_intclkthgh=s.getValOnce(s.eVar2,'ev1',0)
		s.events=s.apl(s.events,'event11',',',1)
	    s.prop33=s.eVar2+':'+s.pageName;
	    }
	   	
	
	/* if a search term is set in prop 1, forse it to lowercase, then copy it to eVar and set a de-duped online search event */
	if(s.prop1){
	...
	/* if a self service is set in prop3, copy to evar3 and set event3 */
	if(s.prop3){
		s.eVar3=s.prop3
	...
	s.prop5=s.getAndPersistValue(s.prop5,'s_UID',365);
	if(s.prop5){
		s.eVar5=s.prop5;
	}
	...
	/* Timeparting EST- Second parameter of function should contain the timezone (GMT) of the report suite: -8=PST; -7=MST; -6=CST; -5=EST */
	s.eVar7=s.prop7=s.getTimeParting('h','-5',new Date().getFullYear()); /* set hour of day */
	s.eVar8=s.prop8=s.getTimeParting('d','-5',new Date().getFullYear()); /* set day of week */

	/* if a Application ID is set in prop9, copy to evar9 */
	if(s.prop9){s.eVar9=s.prop9}
	
	/*populate eVar with previous page*/
	s.eVar10=s.getPreviousValue(s.pageName,'gpv_ev10','');

	/* if a Site Language is set in prop11 copy to evar19*/
	if(s.prop11){s.eVar19=s.prop11}

	/* if a Site Country Code is set in prop12, copy to evar20*/
	if(s.prop12){s.eVar20=s.prop12}
	
	/* if a A/B Testing is set in prop49, copy to evar49*/
	if(s.prop49){s.eVar49=s.prop49}
	
	/*populate prop50 if it has not been set, based on s_account match*/
	if(!s.prop50){
		if(acctmatch.indexOf('scotiabankonline')!=-1){s.prop50="SO";}
		else if(acctmatch.indexOf('scotiabank')!=-1){s.prop50="SB";}
		else {s.prop50="undefined"}
	}

	/* if a siteID is set in prop50, copy to evar50*/
	if(s.prop50){s.eVar50=s.prop50}
	
	/*Custom Prodview event to fire when the prodview event is fired*/
	if(s.events){	
	...
	/* Set up form analysis to track form field abandonment in prop10 */
	if(window.omn_formlist){s.setupFormAnalysis();}
	
	/* page URL */
	s.prop19="D=g";
	
	/* s_code version */
	if(!s.prop51) s.prop51=s_codeVersion;
